    ...
    <tr>
      ...
      <td>Allows to request specific user's details ("claims"). You can choose to receive those claims either in the ID Token (from /token endpoint) or in the UserInfo object (from /userinfo endpoint).<br />It MUST be a JSON object containing an <code>{"id_token":{...}}</code> member or a <code>{"userinfo":{...}}</code> member respectively. This member will then contain all the desired claims - see example below.<br><b>Note</b>: to avoid the need of a /userinfo request, itsme® recommends to retrieve the claims directly from the ID Token.<br><br>Supported claims are listed below.<br />
        <table>
          ...
          <tr>
            <td>{% include parameter.html name="http://itsme.services/v2/claim/birthdate_as_string" req="OPTIONAL" %}</td><td>Returns user's birthdate in an unprocessed way, as mentioned on the ID document. itsme® users are always 16 years old or more.<br><br><b>BE: </b>A value MAY NOT be returned for users with a Belgian ID document. It forms a pair with the <code>birthdate</code> claim: at least one of the 2 SHALL return a value.<br><b>NL: </b>A value SHALL NOT be returned for users with a Dutch ID document.<br><b>LU: </b>A value SHALL NOT be returned for users with a Luxembourgish ID document.</td>
          </tr> 
          <tr>
